...
"""
stolen shamelessly from the RADIATE SDK and used fiendishly for my own personal gain
"""
...
class Sequence:
    ...
    def __init__(self, sequence_path, config_file='config/config.yaml', calib_file='config/default-calib.yaml'):
        """
        ...
        for obj in bboxes_3d:
            ...
            for ii in range(len(bbox_3d)):
                ...
                vis_im = cv2.line(vis_im, (bbox_3d[ii - 1][0], bbox_3d[ii - 1][1]),
                                  (bbox_3d[ii][0], bbox_3d[ii][1]), (np.array(color) * 255).astype(int).tolist(), 1)
